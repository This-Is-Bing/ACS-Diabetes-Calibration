...
raw_STEMI_Atcual_result <- Evaluation(y_validation_ds, pred_prob_valid, rowname = 'STEMI_Atcual_Raw')
...
calibrated_STEMI_Atcual_result <- Evaluation(y_validation_ds, calibrated_prob, rowname = 'STEMI_Atcual_Calibrated')
...
# Removing acsstratum, timiscorestemi, timiscorenstemi
cols_to_remove <- c('acsstratum', 'timiscorenstemi', 'ptoutcome')
calibration_ds <- calibration_ds[, -which(names(calibration_ds) %in% cols_to_remove)]
validation_ds <- validation_ds[, -which(names(validation_ds) %in% cols_to_remove)]

# Normalize outcome
calibration_ds$timiscorestemi <- (calibration_ds$timiscorestemi - min(calibration_ds$timiscorestemi)) / (max(calibration_ds$timiscorestemi) - min(calibration_ds$timiscorestemi))
validation_ds$timiscorestemi <- (validation_ds$timiscorestemi - min(validation_ds$timiscorestemi)) / (max(validation_ds$timiscorestemi) - min(validation_ds$timiscorestemi))

##### Separating features and label ####
X_calibration_ds <- calibration_ds[,-which(names(calibration_ds) == 'timiscorestemi')]
X_validation_ds <- validation_ds[,-which(names(validation_ds) == 'timiscorestemi')]

y_calibration_ds <- calibration_ds[,which(names(calibration_ds) == 'timiscorestemi')]
y_calibration_ds <- ifelse(y_calibration_ds >= 0.5, 1, 0)
y_calibration_ds <- factor(y_calibration_ds)

y_validation_ds <- validation_ds[,which(names(validation_ds) == 'timiscorestemi')]
...
##### Performance Before Calibrating ####
raw_STEMI_TIMI_result <- Evaluation(y_validation_ds, pred_prob_valid, rowname = 'STEMI_TIMI_Raw')
...
##### Performance After Calibrating ####
calibrated_STEMI_TIMI_result <- Evaluation(y_validation_ds, calibrated_prob, rowname = 'STEMI_TIMI_Calibarted')
