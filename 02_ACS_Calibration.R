...
##### Evaluate model ####
acs_raw_result <- Evaluation(y_calibration_ds, pred_prob_calib, raw_threshold, rowname = 'ACS_Raw')
...
# Using Best F1 Score for Threshold
calib_threshold <- SearchBestThreshold(y_calibration_ds, calibrated_prob)

##### Evaluate Model ####
acs_calibrated_result <- Evaluation(y_calibration_ds, calibrated_prob, calib_threshold, rowname = 'ACS_Calibrated')
...
# write.csv(final_result, "./results/2/calibrated_results/ACS_Calibration_Valid_F1.csv")

##### Exporting Model ####
acs_calibrated_model <- res$cal.model
# saveRDS(calibrated_model, file = "./results/2/calibrated_models/acs_calibrated_model.rds")
