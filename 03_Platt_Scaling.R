...
pred_prob_calib$ptoutcome <- as.numeric(calibration_ds$ptoutcome== 0)
pred_prob_valid$ptoutcome <- as.numeric(validation_ds$ptoutcome == 0)

#### HL Test for testing dataset ####
hoslem.test(pred_prob_testing$ptoutcome,
            pred_prob_testing$`predict(model, X_test_testing, type = "prob")$Death`, 
            g=10)

#-squared = 6920.8, df = 8, p-value < 2.2e-16
...
hoslem.test(ptoutcome,pred_risk, g=10)
